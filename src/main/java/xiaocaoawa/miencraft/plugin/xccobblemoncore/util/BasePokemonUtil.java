 ...
 * @version 1.0
 */
public class BasePokemonUtil {

    private static final int IV_SUM = IVs.MAX_VALUE * 6;

    // 私有构造器防止实例化
    private BasePokemonUtil() {}

    /**
     * 判断当前是否为空对象（即 pokemon 是否为 null）
     *
     * @return 如果 pokemon 为 null 返回 true，否则返回 false
     */
     ...
     *
     * @return 宝可梦的物种对象
     * @throws NullPointerException 如果 pokemon 为 null
     */
     ...
     * @return 返回主属性类型（ElementalType），如“火”、“水”等。
     ...
     * 获取宝可梦的副属性类型。
     *
     * 副属性是部分宝可梦拥有的第二个属性，若该宝可梦没有副属性，则返回 null。
     * 例如：飞行系、格斗系等。
     *
     * @param pokemon 宝可梦对象，不能为 {@code null}。
    ...
    public static String getMoveDescription(@NotNull Pokemon pokemon, int i) {
        Move move = getMove(pokemon, i);
        if (move == null) return PokemonUtil.none;
    ...
    public static String getOriginalTrainerName(@NotNull Pokemon pokemon) {
        ...
        return originalTrainerName == null ? PokemonUtil.yes : originalTrainerName;
    }
     ...
     *
     * @return 返回 NPCEntity 对象，如果宝可梦不是由 NPC 拥有则返回 null
     * @throws NullPointerException 如果当前宝可梦对象为 null
     */
    ...
    /**
     * 获取宝可梦持有者的实体对象（适用于非玩家或 NPC 的其他实体类型）。
     *
     ...
     * @throws NullPointerException 如果宝可梦对象为null，则抛出空指针异常，并提供异常消息"Pokemon is null"
     */
